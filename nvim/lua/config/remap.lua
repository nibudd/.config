...
end, { desc = "yank absolute filepath to register f" })
...
vim.keymap.set("n", "<leader>gD", "<cmd>DiffviewOpen origin/main...HEAD<CR>", { desc = "open diffview against main" })

vim.keymap.set(
	"n",
	"<leader>lq",
	"<cmd>call setloclist(0, [], ' ', {'items': get(getqflist({'items': 1}), 'items')})<CR><cmd>cclose<CR><cmd>lopen<CR>",
	{ desc = "copy quickfix list to local list" }
)
